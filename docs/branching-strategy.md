...
1. Helps with onboarding since structure aligns with a versioned release cycle
    1. Makes it easier to see where to tag
    1. Adds redundancy when marking versions
1. Helps with collective project velocity since the dedicated staging branches don't block the integration of non-release PRs during a release phase (~1 week)
1. Helps let the CI be more efficient by allowing phase-based actions
1. Protects the most stable branch by inherently requiring multiple checkpoints:
    1. Development -> Integration: Reviews to ensure that new changes work in isolation
    1. Integration -> Staging: Testing to ensure that all changes since last release work together and make minimal adjustments as needed
    1. Staging -> Release: Sign off that next released has passed
...
- `(development|user)/#-(feature|fix|refactor|etc)-*`: mortal branches where contributors can develop in isolation
